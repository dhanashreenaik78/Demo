...
public class LatexAss
{
	public static void main(String args[])
	{
	try
	{
	String filename = "E:\\practice\\excel\\LatexQuestion.xlsx";
...
// 	   res=aval+(nval-1)*dval;
 	      ...
 	      int a = (nval-1)*dval;
 	     int res1 = aval+a;
         String dt = "We know $t_n = a+(n-1)\times d $ so $a = t_n -(n-1)\times d $ here we have d = "+dval+ " n = "+nval+ "and a ="+aval+ " now substituting value we get $"+ "$  then $t_{"+nval+"} ="+aval+"$  $+("+nval+"$ -1)$\times"+dval+"$  is : " +res1;
